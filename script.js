...
/* ==========================================================
   ...
   ETAPA 5 - SALVAMENTO SEGURO DOS REGISTROS NO SUPABASE
...
const PHOTO_TARGET_BYTES = 320 * 1024;
...
async function prepararSessao(session, options = {}) {
  ...
  try {
    ...
    if (userError || !userData?.user) {
      throw userError || new Error("Usuário autenticado não encontrado.");
    }
    ...
    const { data: colaborador, error: perfilError } = await state.supabase
      .from("colaboradores")
      .select(
        "id, auth_user_id, empresa_id, nome_completo, cpf, email, telefone, cargo_funcao, perfil, administrador_global, status, data_admissao, foto_perfil_url"
      )
    ...
    const { data: empresa, error: empresaError } = await state.supabase
      .from("empresas")
    ...
    const { data: configuracao, error: configuracaoError } = await state.supabase
      .from("configuracoes")
      .select(
...
async function carregarRegistrosDeHoje({ showToast = false } = {}) {
  ...
  try {
    ...
    const { data, error } = await state.supabase
      ...
      .select("id, tipo, registrado_em, data_referencia, codigo_registro, status")
...
async function compactarFonteVisual(source, sourceWidth, sourceHeight, options = {}) {
  ...
  while (blob.size > PHOTO_TARGET_BYTES && Math.max(width, height) > 720) {
...
async function compactarCanvas(canvas) {
  const qualities = [0.78, 0.7, 0.62, 0.54, 0.48];
  let lastBlob = null;

  for (const quality of qualities) {
    lastBlob = await canvasParaBlob(canvas, "image/jpeg", quality);

    if (lastBlob.size <= PHOTO_TARGET_BYTES) {
      break;
    }
  }

  if (!lastBlob) {
    throw new Error("Falha ao gerar o arquivo JPEG.");
  }
...
async function registrarPontoNoSupabase() {
  ...
  try {
    mostrarMensagemSalvamento(
      "Validando a sessão e preparando o arquivo protegido...",
      "info"
    );
    ...
    if (userError || !userData?.user) {
      throw userError || new Error("Sessão inválida. Entre novamente.");
    }

    if (userData.user.id !== state.authUser?.id) {
      throw new Error("A sessão atual não corresponde ao colaborador carregado.");
    }
    ...
    if (!objectPath) {
      mostrarMensagemSalvamento(
        "Enviando a foto para o armazenamento privado...",
        "info"
      );
    ...
    mostrarMensagemSalvamento(
      "Foto protegida. Registrando o horário oficial no Supabase...",
      "info"
    );

    const fotoReferencia = `${PHOTO_BUCKET}/${objectPath}`;

    const { data, error } = await state.supabase.rpc("registrar_ponto", {
...
function validarDadosParaRegistro() {
  ...
  if (state.photoBlob.type !== "image/jpeg") {
    return "A foto precisa estar no formato JPEG.";
  }

  if (state.photoBlob.size > 1024 * 1024) {
    return "A foto ultrapassou 1 MB. Tire outra foto para que seja comprimida novamente.";
  }
...
function construirCaminhoDaFoto(pointType) {
  ...
  return [
    ...
    parts.month,
    parts.day,
    `${timestamp}-${safeType}-${randomId}.jpg`,
  ].join("/");
}
...
function obterInformacoesDoDispositivo() {
  return {
    ...
    largura_tela: window.screen?.width || null,
    altura_tela: window.screen?.height || null,
    ...
    foto_largura: state.photoMeta?.width || null,
    foto_altura: state.photoMeta?.height || null,
...
async function localizarRegistroAposFalha(pointType) {
  try {
    ...
    const { data, error } = await state.supabase
      .from("registros_ponto")
      .select(
        "id, tipo, registrado_em, data_referencia, latitude, longitude, precisao_metros, distancia_empresa_metros, dentro_area_permitida, foto_url, foto_provedor, codigo_registro, status"
      )
      .eq("colaborador_id", state.colaborador.id)
      .eq("data_referencia", today)
      .eq("tipo", pointType)
